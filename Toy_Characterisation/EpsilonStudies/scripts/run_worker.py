...
# ═══════════════════════════════════════════════════════════════════
#  Task: param_opt
#  Hamiltonian parameter optimisation sweep (ERF convolution only).
#  Sweeps over (k, theta_d, measurement_error, collision_noise)
#  and measures reco metrics for each (angle, n_tracks).
#  Saves full per-repeat data for reproducibility.
# ═══════════════════════════════════════════════════════════════════

# Metric keys to extract from EventValidator.compute_metrics()
PARAM_OPT_METRIC_KEYS = [
...
def _serialise_event(event):
    """Extract event data into plain numpy/list structures for pickling."""
    hits = []
    for h in event.hits:
        hits.append({
            'hit_id': h.hit_id, 'x': h.x, 'y': h.y, 'z': h.z,
            'module_id': h.module_id, 'track_id': h.track_id,
        })
    truth_tracks = []
    for t in event.tracks:
        truth_tracks.append({
            'track_id': t.track_id,
            'hit_ids': list(t.hit_ids),
        })
    return {'hits': hits, 'truth_tracks': truth_tracks}


def _serialise_reco_tracks(reco_tracks):
    """Extract reco track data into plain list structures."""
    out = []
    for t in reco_tracks:
        out.append({
            'track_id': t.track_id,
            'hit_ids': list(t.hit_ids),
        })
...
def run_param_opt(params, outdir):
    ...
    per_repeat = []
    repeat_data = []          # full data for reproducibility
    ...
    for rep in range(n_repeats):
        ...
        # Save Hamiltonian matrix A in COO form for efficient storage
        A_coo = sp.coo_matrix(ham.A)
        segments_pairs = [(s.hit_start.hit_id, s.hit_end.hit_id)
        ...
        if len(reco_tracks) == 0:
            row = {mk: 0.0 for mk in PARAM_OPT_METRIC_KEYS}
            full_metrics = {}
        else:
            val = EventValidator(event, reco_tracks)
            full_metrics = val.compute_metrics()
            row = {mk: float(full_metrics.get(mk, 0.0))
                   for mk in PARAM_OPT_METRIC_KEYS}
        ...
        # Store full data for this repeat
        repeat_data.append({
            'repeat':        rep,
            ...
            'b':             np.array(ham.b),
            'solution':      np.array(x),
            'segments':      segments_pairs,
            'reco_tracks':   _serialise_reco_tracks(reco_tracks),
    ...
    # ── Save reproducibility archive ──────────────────────────────
    with gzip.open(outdir / "repeat_data.pkl.gz", 'wb') as f:
        pickle.dump(repeat_data, f, protocol=pickle.HIGHEST_PROTOCOL)

  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \begin{itemize}
        ...
        \item HL-LHC pile-up (many more simultaneous collisions) $\Rightarrow$ reconstruction cost threatens to outpace classical resources.
    ...
    \begin{column}{0.46\textwidth}
      ...
      \cn{A generated toy VELO event: tracks fan from the collision point (primary vertex) through 5 planes.}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \begin{enumerate}
        ...
        \item \textbf{QPE} (quantum phase estimation): $e^{iAt}$ writes each eigenvalue $\lambda_k$ onto an $n_{\rm time}$-qubit clock register;
      ...
      \textbf{Catch:} dense $e^{iAt}$ + high-precision QPE $\Rightarrow$ prohibitive depth on near-term hardware.
    \end{column}
...
\begin{frame}{Why the 1BQF wins --- in brief}
  \begin{itemize}
    ...
    \item \textbf{NISQ-validated} (today's noisy devices): Quantinuum H2 and IBM Heron noise models.
...
\begin{frame}{Add a bifurcation (fork) penalty to the Hamiltonian}
  ...
  {\footnotesize $\varepsilon$-windowed $B_\varepsilon$ (only near-collinear competitors): \textbf{false rate $\to0$ at $\sim$2\% efficiency cost}, sparse, 1BQF-safe (AUC $0.96$--$0.99$); the dense fork would break the filter [backup].}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \begin{itemize}
        ...
        \item First results: a degree-$\sim$30 polynomial lifts true/false AUC from $0.49$ (classical) / $0.64$ (1-bit) to $\mathbf{0.88}$ [backup].
      \end{itemize}
    \end{column}
    \begin{column}{0.46\textwidth}
...
\appendix
\section*{Backup}
% ---- demoted from the main line for the 30-minute slot ----
\begin{frame}{Backup: a controlled toy VELO --- our testbed}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
...
\begin{frame}{Backup: the toy injects the two physical detector noises}
  \centering
...
\begin{frame}{Backup: HHL stage by stage --- the statevector at every step}
  \centering
...
\begin{frame}{Backup: the conceptual reformulation --- inversion $\to$ filtering}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{itemize}
...
\begin{frame}{Backup: class by class at matched efficiency ($T{=}400$)}
...
\begin{frame}{Backup: the fork term --- dense breaks the quantum, $\varepsilon$-windowed fixes it}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
...
\begin{frame}{Backup: QSVT --- a polynomial filter lifts true/false separation}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}

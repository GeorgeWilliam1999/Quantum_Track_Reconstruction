"""make_schematic.py — the labeled worked-example figure for the bifurcation note.
...
Draws ONE figure with three panels that take you from picture -> matrix:
  ...
  (b) the two segment graphs that the picture defines — the continuation graph C
      (opposite-side hit sharing, the -1 chain couplings) and the fork graph B
      (same-side hit sharing, the +beta fork coupling);
  (c) the assembled off-diagonal Hamiltonian A' = (gamma+delta) I - C + beta B,
...
# h1p is the extra plane-1 hit that creates the false fork.
hits = {
    "h0": (0, 0.00), "h1": (1, 0.35), "h2": (2, 0.70),
    "h3": (3, 1.05), "h4": (4, 1.40), "h1'": (1, -0.55),
}
...
# fork B: share START hit or share END hit (same side)
B = np.zeros((n, n), int)
for a in names:
    for b in names:
...
for z in range(5):
    ...
    axA.text(z, -1.05, f"plane $M_{z}$", ha="center", va="top", fontsize=10, color=GREY)
...
# false fork (red dashed)
arrow(axA, "h0", "h1'", RED, ls="--")
axA.text(0.5, -0.40, "$f$", color=RED, fontsize=12, ha="center",
...
for h, (z, y) in hits.items():
    ...
    dy = -0.16 if h == "h1'" else 0.13
    va = "top" if h == "h1'" else "bottom"
    axA.annotate(f"${h}$", (z, y), textcoords="offset points",
                 xytext=(7, 9 if va == "bottom" else -11), fontsize=10.5, color=col)

# highlight the shared start hit h0
axA.add_patch(Circle(hits["h0"], 0.16, fill=False, ec="darkorange", lw=2.4, zorder=4))
axA.annotate("shared start-hit $h_0$\n$\\Rightarrow$ out-fork  $(s_1,f)$",
             hits["h0"], textcoords="offset points", xytext=(-6, -52),
...
axA.set_title("(a)  Detector schematic: a true 4-segment chain $s_1\\!-\\!s_2\\!-\\!s_3\\!-\\!s_4$ "
              "and a false fork $f$ sharing $s_1$'s start-hit", fontsize=11.5, loc="left")
axA.set_xlim(-0.6, 4.6); axA.set_ylim(-1.5, 1.9)
...
axB.plot([], [], "--", color=RED, lw=2.2, label=r"fork $B$  (share same-side hit, $+\beta$)")
axB.legend(loc="lower right", fontsize=8.6, frameon=False)
...
axC.set_title(r"(c)  $A' = (\gamma{+}\delta)\,I - C + \beta B$   "
...
fig.suptitle("Bifurcation term — from labeled picture to Hamiltonian matrix",
